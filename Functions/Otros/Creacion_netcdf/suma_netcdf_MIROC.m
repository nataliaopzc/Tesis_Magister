...
addpath(fullfile(userdir, 'Dropbox','MIROC-ESM'))

    T_dry=ncread('drydust_aero_MIROC-ESM_PiControl_r1i1p1_T42_monthly.nc','drydust');
    T_wet=ncread('wetdust_aero_MIROC-ESM_PiControl_r1i1p1_T42_monthly.nc','wetdust');
    A(:,:,:,1)=[T_dry + T_wet];
    T_dry=ncread('drydust_aero_MIROC-ESM_lgm_r1i1p1_T42_monthly.nc','drydust');
    T_wet=ncread('wetdust_aero_MIROC-ESM_lgm_r1i1p1_T42_monthly.nc','wetdust');
    A(:,:,:,2)=[T_dry + T_wet];


    %% Creando NETCDF
lon1=ncread('drydust_aero_MIROC-ESM_lgm_r1i1p1_T42_monthly.nc','lon');
lat1=ncread('drydust_aero_MIROC-ESM_lgm_r1i1p1_T42_monthly.nc','lat');
%Takemura=load('Takemura.nc_annualflux_kgperm2pers.128x64_LGM.dat');

file='/home/naty/Dropbox/MIROC-ESM/MIROC-ESM.nc';
lon='lon';
lat='lat';
...
netcdf.putAtt(ncid,dataID,'long_name', 'Global Interpolated Dust Fluxes MIROC-ESM Holoceno-LGM');
