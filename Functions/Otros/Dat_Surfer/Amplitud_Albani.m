...
z_Holoceno=ncread('Albanietal2014_C4fn_dep.nc','dep')./(1000*3600*24*365);
z_LGM=ncread('Albanietal2014_C4fn-lgm_dep.nc','dep')./(1000*3600*24*365);

lat=ncread('Albanietal2014_C4fn_dep.nc','lat');
lon=ncread('Albanietal2014_C4fn_dep.nc','lon');
Razon=log10(z_LGM./z_Holoceno); % En el LGM se espera más polvo que en el Holoceno. 
Z=Razon(:);
Lat=[];
for i=1:length(lat);
...
dlmwrite(fullfile(Dir,'netcdf',['Albani_Amplitud.dat']),OUTPUT(),'precision','%14.6e','delimiter','');
