...
for i=1:10
    Dust(:,:,i)=load(fullfile(userdir,'Dropbox','Polvo_Region_MIROC-ESM',['MIROC-ESM.nc_annualflux_molperyr.36x36_level' num2str(i) '.dat']));
end
...
% Save Results
  ...
  for i=1:10;
      dlmwrite(fullfile(userdir,'Dropbox','DustRegions_MIROC-ESM',['MIROC-ESM_' region '_Level' num2str(i) '.dat']),OUTPUT(:,:,i),'precision','%14.6e','delimiter','');
  end
end
