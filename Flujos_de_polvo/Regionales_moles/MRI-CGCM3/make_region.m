...
for i=1:10
    Dust(:,:,i)=load(fullfile(userdir,'Dropbox','Polvo_Region_MRI-CGCM3',['MRI-CGCM3.nc_annualflux_molperyr.36x36_level' num2str(i) '.dat']));
end
...
% Save Results
  ...
  for i=1:10;
      dlmwrite(fullfile(userdir,'Dropbox','DustRegions_MRI-CGCM3',['MRI-CGCM3_' region '_Level' num2str(i) '.dat']),OUTPUT(:,:,i),'precision','%14.6e','delimiter','');
  end
end
